    ...
    try:
        ...
        for feature in data:
            for element in feature.get("elements", []):
                # Only count Scenario/Scenario Outline elements (skip Background)
                if element.get("type") not in ("scenario",):
                    continue
                scenario_name = element.get("name", "unknown")
                steps = element.get("steps", [])

                step_statuses = [s.get("result", {}).get("status", "unknown") for s in steps]

                if any(s == "failed" for s in step_statuses):
                    scenarios_failed += 1
                    error_msg = next(
                        (s.get("result", {}).get("error_message", "")
                         for s in steps if s.get("result", {}).get("status") == "failed"),
                        "",
                    )
                    failures.append({
                ...
                elif all(s == "passed" for s in step_statuses) and step_statuses:
                    scenarios_passed += 1
                else:
                    # undefined, pending, skipped — scenario is pending
                    scenarios_pending += 1

        total = scenarios_passed + scenarios_failed + scenarios_pending
        return {
            "status": "PASS" if scenarios_failed == 0 and total > 0 else ("FAIL" if scenarios_failed > 0 else "NO_RUNS"),
            "passed": scenarios_passed,
            "failed": scenarios_failed,
            "pending": scenarios_pending,
            "total": total,
...
# ---------------------------------------------------------------------------
# GET /automation/report/html — serve the Cucumber HTML report
# ---------------------------------------------------------------------------

@router.get("/report/html")
def get_html_report():
    """Serve the latest Cucumber HTML report generated by @cucumber/html-formatter.

    The report is written to automation/reports/bat-report.html by ``npm run test:bat``.
    Falls back to cucumber-report.html if present.
    """
    from fastapi.responses import FileResponse, HTMLResponse

    for candidate in ("bat-report.html", "cucumber-report.html"):
        html_path = _REPORTS_DIR / candidate
        if html_path.exists():
            return FileResponse(
                str(html_path),
    ...
    # No HTML report yet — return 200 with a styled stub so the iframe renders
    return HTMLResponse(
        content=(
            "<!doctype html><html><head><meta charset='utf-8'>"
            "<style>body{margin:0;display:flex;align-items:center;justify-content:center;"
            "min-height:100vh;font-family:system-ui,sans-serif;background:#0f0f0f;color:#888}"
            ".box{text-align:center;padding:2rem}.title{font-size:1.1rem;font-weight:600;"
            "color:#ccc;margin-bottom:.5rem}.sub{font-size:.85rem}code{background:#1e1e1e;"
            "color:#4ec9b0;padding:.1em .4em;border-radius:4px;font-size:.8rem}"
            "</style></head><body><div class='box'>"
            "<div class='title'>No HTML report yet</div>"
            "<div class='sub'>Run <code>npm run test:bat</code> or "
            "<code>npm run test:regression</code> inside <code>automation/</code> "
...
def trigger_run(body: RunRequest, background_tasks: BackgroundTasks):
    ...
    if body.use_docker:
        ...
        if body.tags:
            inner_cmd = (
                f"npx cucumber-js --config cucumber.conf.js --tags '{body.tags}'"
...
def _build_run_cmd(tags: str, use_docker: bool) -> list[str]:
    ...
    if use_docker:
        ...
        if tags:
            inner = (
                f"npx cucumber-js --config cucumber.conf.js --tags '{tags}'"
                " --format json:reports/cucumber-report.json"
            )
