...
type Tab = 'health' | 'results' | 'traces' | 'run' | 'html_report'
...
export default function AutomationHealthPage() {
  ...
  const tabs: { id: Tab; label: string; icon: React.ElementType }[] = [
    ...
    { id: 'html_report', label: 'HTML Report', icon: ExternalLink },
  ]
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-1 border-b overflow-x-auto scrollbar-none">
        {tabs.map(t => (
          <button
            ...
            className={cn(
              'flex shrink-0 items-center gap-1.5 px-3 py-2 text-sm font-medium border-b-2 -mb-px transition-colors',
              activeTab === t.id
      ...
      {/* ── Tab: HTML Report ── */}
      {activeTab === 'html_report' && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <p className="text-sm text-muted">
              Cucumber HTML report — generated by{' '}
              <code className="text-xs bg-accent px-1 py-0.5 rounded">npm run test:bat</code> or{' '}
              <code className="text-xs bg-accent px-1 py-0.5 rounded">npm run test:regression</code>.
            </p>
            {selectedEndpoint && (
              <a
                href={APIRoutes.AutomationReportHtml(selectedEndpoint)}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 text-xs text-brand hover:underline shrink-0 ml-4"
              >
          ...
          {!selectedEndpoint ? (
            <div className="rounded-xl border border-accent bg-primaryAccent p-8 text-center text-sm text-muted">
              Connect to an API endpoint first.
            </div>
          ) : (
            <div
              className="rounded-xl border border-accent overflow-hidden bg-[#0f0f0f]"
              style={{ height: '72vh' }}
            >
              <iframe
                key={selectedEndpoint}
                src={APIRoutes.AutomationReportHtml(selectedEndpoint)}
